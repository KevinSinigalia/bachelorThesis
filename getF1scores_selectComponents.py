...
#same principle as in other F1-scores scripts. Read other scripts comments for explanation
                ...
                for i, ensemble in enumerate(link_mat_dic[season][dataset]):
                    if (dataset!=ref_ds or dataset==ref_ds) and dataset!='ncar':
                        ...
                        for j, ref_ds_ensemble in enumerate(link_mat_dic[season][ref_ds]):                      
                            ...
                            tau_min=1, tau_diff=0, same_sign=True)
...
alphaLevelList=[1e-1,5e-1, 1e-2, 5e-2, 1e-3, 5e-3, 1e-4, 5e-4, 1e-5, 3e-5, 7e-5, 1e-6, 3e-6, 7e-6, 1e-7, 3e-7, 7e-7, 1e-8, 5e-8, 1e-9, 1e-10]
pca_res_path="./c-transfer"
pcmci_res_path="./output/mapSearch"
...
for folder in os.listdir(pcmci_res_path):
    ...
    pattern = r"alpha=([0-9.]+)_nVAR=(\d+)_hash=(-?\d+)"
    ...
    file_name='./output_f1-scores_lag1/mapSearchOldTaumin1/f1Scores_PCalpha=%f_nVAR=%d_hash=%d.bin' % (pc_alpha, n_kept_comp, hash_value)
    ...
    for alpha_levelTest in scattered_jobs:
        ...
        for res_file in glob.glob(pcmci_res_path_+"/results_*.bin"):       
            ...
            selected_comps_indices=resTest1['variables']                      #----------!!! get selected variables as in resDict specified !!!---------
    ...
    if rank == 0:
        ...
        'selected_comps': selected_comps_indices,
        'f1Scores': resDict}
